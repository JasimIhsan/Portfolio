...
export function useThemeToggle({ variant = "circle-blur", start = "top-right" }: { variant?: ThemeVariant; start?: RectStart } = {}) {
   ...
   const toggle = (e?: React.MouseEvent | React.TouchEvent | MouseEvent | TouchEvent) => {
      ...
      if (variant === "rectangle") {
         ...
         let origin = CIRCLE_ORIGIN[start];

         // Calculate origin from button coordinates if event or active target is present
         if (e) {
            ...
            // If event had target element, get bounding rect center for perfect origin
            const currentTarget = (e.currentTarget || e.target) as HTMLElement | null;
...
export function ThemeToggle({ variant = "circle-blur", start = "top-right", className, iconClassName = "h-4 w-4", ...rest }: ThemeToggleProps) {
   const { isDark, mounted, toggle } = useThemeToggle({ variant, start });

   return (
      <button type="button" aria-label={mounted && isDark ? "Switch to light mode" : "Switch to dark mode"} onClick={(e) => toggle(e)} className={cn("flex items-center justify-center cursor-pointer transition-colors", className)} {...rest}>
         {mounted ? (
            <ActionSwapIcon value={isDark ? "dark" : "light"} animation="blur" className={iconClassName}>
               {isDark ? <Sun className={cn(iconClassName, "text-[#D4A373]")} /> : <Moon className={cn(iconClassName, "text-[#8A5A2B]")} />}
            </ActionSwapIcon>
         ) : (
